...
- **Добавлены флажки для строк таблицы**  
Теперь вы можете выбирать конкретные элементы для экспорта.

- **Добавлены фильтры поиска**  
Настройте, в каком столбце таблицы выполнять поиск (по названию, количеству, единицам измерения)

![Filters Menu](screenshots/filters_menu.png)

### 🖼️ Дизайн

- Обновлен общий вид таблицы.
- Поменяны местами кнопки Экспорта и создания документа для более интуитивного доступа.
- Переработано всплывающее окно поиска для большей наглядности и удобства использования.
- Незначительные улучшения в различных элементах пользовательского интерфейса.

### 🐞 Исправление ошибок

- Исправлены проблемы с локализацией в интерфейсе.
- Устранена ошибка, из-за которой кнопка создания документа не отключалась во время экспорта.
...
- Настраиваемые фильтры поиска (название, количество, единицы измерения)
...
- После выбора изделия (например, *БКН.0.12*):  
  `[✓] Поиск по материалам — БКН.0.12`  
...
- Формат `.xlsx`  
- Экспорт материалов, отмеченных галочкой
...
Настоящим предоставляется бесплатное разрешение любому лицу, получившему копию этого программного обеспечения и сопутствующих файлов с документацией...
